...
\begin{figure}[h]
    ...
    \label{fig:OneFracture}
\end{figure}
In this case the molecular diffusion is the same as the effective diffusion.
...
Firstly, a network of fractures characterised by a cubic building block is created. Pores and throats diameters and throat length is the same as for the 1D case. The diffusive conductance values of the throats are sampled from a lognormal distribution with standard deviation $s=0.5$, the location parameter $loc=0$ and the scale factor $scale=D=10^{-3}$. Figure \ref{fig:Network} represents the cubic pore-throat network and the line width is proportional to the diffusive conductance of the throats.
\FloatBarrier  % Prevents figures from floating past this point
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/fromOpenPNM/rectangular2Dpn.png}
    \caption{2D network of fractures. The width of the lines is proportional to the diffusivity of the throats}
    \label{fig:Network}
\end{figure}
\FloatBarrier  % Prevents figures from floating past this point
\begin{figure}[h]
    ...
    \label{fig:LogDiff}
\end{figure}
Secondly, the flow rate $Q^{tot}_D$ is computed using the \texttt{FickianDiffusion} from OpenPNM and the spatial distribution of the concentration is reported in figure \ref{fig:ConcInFractures}.
\FloatBarrier  % Prevents figures from floating past this point
\begin{figure}[h]
    \centering
...
\end{figure}
Eventually, the effective diffusion can be calculated using \ref{eq:EffDiff}

the flow rate is $Q_{tot}=0.00043$ $m^3/s$. Since the cross-section area of all the throats at the outlet is $3.926*10^{-8}$ $m^2$, it follows that the effective diffusion is $D_{eff}=11036$ $m/s$. Should it be multiplied by which throat length?
