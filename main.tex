...
The statistical characterisation of fractures according to \ref{clemo1994dual} divides the relevant parameters into three groups: 1) characteristics of individual fractures, 2) fracture density which is a composite characteristic of all the fractures, and 3) characteristics of the relationships of individual fractures. These are listed in table \ref{}:
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/Clemo1994FracChar.png}
    \caption{BTC from simulation.}
    \label{tab:FracChar}
\end{figure}
The most relevant parameters for simulating hydrogen diffusion through fractured domain are listed in table \ref{tab:dfnWorksParam} with some typical range for fractured shale caprock:
\begin{table}[htbp]
\centering
\small
...
\midrule
\texttt{domainSize} & Size of the cubic simulation domain & $[5, 5, 5]$ m (minimum representative volume) \\
...
\paragraph{Effective Diffusion Coefficient:} finally, the effective diffusion coefficient \( D_{\text{eff}} \) can be computed using the dispersion coefficient and the fluid velocity \( v \) as:
\[ D_{\text{eff}} = \frac{D_{\text{disp}}}{v} \]
...
\label{tab:StatMomV4}
\end{table}
The dispersion coefficient is \[ D_{\text{disp}} = \frac{E[t^2]}{2 E[t]} = 336. \]
